...
export const ThreePreview = forwardRef<ThreePreviewHandle, Props>(
  function ThreePreview({ sceneData, activeLayerId, guideVisible }, ref) {
    ...
    useEffect(() => {
      ...
      renderer.setAnimationLoop(() => {
        ...
        sortedLayers.forEach((layer, order) => {
          ...
          const [x, y, z] = computeLayerWorldPosition(
            layer.latitude,
            layer.longitude,
            radius,
          );
          ...
          item.material.color.set(
            layer.id === activeLayerIdRef.current ? 0xd9e4ff : 0xffffff,
          );
        });
        ...
        scene.traverse((object: any) => {
          if (object instanceof Mesh) {
            ...
            if (Array.isArray(object.material)) {
              object.material.forEach((material: MeshBasicMaterial) =>
                material.dispose(),
              );
            } else {
    ...
    useEffect(() => {
      ...
      applySphereTexture(
        material,
        resolveGuideImageUrl(sceneData.guideImageUrl),
        0xffffff,
        0.28,
      );
